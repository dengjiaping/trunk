            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (itemAdapter instanceof EmotionGridViewAdapter) {
                    ...
                    if (position == emotionGvAdapter.getCount() - 1) {
                        ...
                        if (ViewUtils.getMaxLength(mEditText)<=0||ViewUtils.getMaxLength(mEditText)> sb.toString().length()) {
                            // 特殊文字处理,将表情等转换一下
                            ...
                            Toast.makeText(mContext, "文字长度超出范围，请先删除部分内容后再操作！", Toast.LENGTH_LONG).show();
                        }
                    }
                }
            }
