...
public class WelcomeActivity extends BaseActivity {
    ...
    private void loadSplashData() {
        if (IntenetUtil.getNetworkState() > 0) {
            ...
            HiStudentHttpUtils.postDataByOKHttp(WelcomeActivity.this, map_sign, HistudentUrl.update_apk_url, new HttpRequestCallBack() {
                @Override
    ...
    private void doYunXinAutomaticLogin() {
        ...
        if (model == null) {
            ...
            Logger.e("YUNXIN--->token:" + model.getToken());
            ...
            NIMClient.getService(AuthService.class).login(info).setCallback(new RequestCallbackWrapper() {
                ...
                public void onResult(int i, Object o, Throwable throwable) {
                    if (isPageFinish) return;
                    Logger.e("Yun Xin Start Login------>back:i===" + i);
    ...
    private void doLoginAction() {
        ...
        if (IntenetUtil.getNetworkState() > 0) {
            if (mLoginType == 1 && !TextUtils.isEmpty(HiCache.getInstance().getUserLoginToken())) {
                BodyActivity.start(WelcomeActivity.this);
            } else {
                LoginActivity.start(WelcomeActivity.this);
            }
            isPageFinish = true;
    ...
    private void solveNetworkStatus() {
        ...
        ReminderHelper.getIntentce().showDialog(this, "提示", "当前网络不可用，请连接网络后重试", "退出", () -> WelcomeActivity.this.finish(), "设置网络", () -> {
            Intent intent;
